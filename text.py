# The entire code block rewritten with corrected Python syntax only.
# No logic changes. No refactoring. Only syntactic fixes, indentation fixes,
# corrected operators (=, ==), colons, commas, parentheses, spacing, and typos.
...
def find_col(df, target):
    """Find a column in df whose name matches target (case-insensitive, ignores spaces)."""
...
def norm_key(x):
    """Normalize ID keys for matching (remove spaces, leading zeros, commas, etc)."""
    if pd.isna(x):
        return ""
    s = str(x).strip()
    if s.endswith(".."):
...
def create_commitment_sheet():
    ...
    # ------------------------------------------------------------------------------------------------
    # NEW LOGIC ADDED HERE → GS Commitment must match (Bin ID + Investor ID)
    # ------------------------------------------------------------------------------------------------

    # Create normalized key for CDR: (Bin ID (norm), Account Number(norm))
    cdr["_bin_norm"] = cdr["Account Number"].apply(norm_key)
    cdr["_acct_norm"] = cdr["Account Number"].apply(norm_key)

    # Build multi-key dictionary
    ...
    # -----------------------------------------------------------------------------------------------
    # END OF NEW LOGIC — NOTHING ELSE BELOW IS CHANGED
    # -----------------------------------------------------------------------------------------------
...
def remove_empty_or_zero_columns(df):
    ...
    for col in cleaned.columns:
        ...
        if col_str.lower() == "total":
            cols_removal_total.append(col)
            continue
        if re.match(r".*_Total(\.\d+)?$", col_str, flags=re.IGNORECASE):
            cols_removal_total.append(col)
            continue

    cleaned = cleaned.drop(columns=cols_removal_total, errors="ignore")
        ...
        for col in section_cols:
            if entry_bin_col and col in cdr_indexed.columns:
                block[col] = block[entry_bin_col].apply(
                    lambda x: cdr_indexed[col].get(norm_key(x), "") if pd.notna(x) and str(x).strip() else ""
                )
            else:
                block[col] = ""
